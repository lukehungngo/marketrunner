...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        all_meta_stock_data = apps.get_app_config("app").get_btc_all_data_1d()
        len(all_meta_stock_data)
        num_of_date_before = 30 * 80
        meta_stock_data = all_meta_stock_data.tail(num_of_date_before)
        print(meta_stock_data.tail(1))
        ...
        meta_stock_forecast["Date"] = pd.to_datetime(
            meta_stock_forecast["Date"]
        )  # Ensure 'Date' is in datetime format
        # Plotting
        plt.figure(figsize=(10, 6))
        plt.plot(
            meta_stock_forecast["Date"],
            meta_stock_forecast["TimeGPT"],
            marker="o",
            linestyle="-",
            color="b",
        ...
        plt.title(
            "{} Price Forecast Using data from {}".format(
                "BTC-USD",
                (datetime.now() - timedelta(days=num_of_date_before)).strftime(
                    "%Y-%m-%d"
                ),
            )
        )
